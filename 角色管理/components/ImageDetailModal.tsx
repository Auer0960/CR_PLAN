...
const ImageDetailModal: React.FC<ImageDetailModalProps> = ({ isOpen, onClose, image, character, tagCategories, allTags, onUpdateImage, onDeleteImage, onAddTagToCategory, currentUser }) => {
  ...
  const displayName = character?.name || '未知角色';
      ...
      <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={onClose}>
        <div className="bg-gray-800 text-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col md:flex-row overflow-hidden" onClick={e => e.stopPropagation()}>
          ...
          <div className="w-full md:w-2/3 flex-shrink-0 bg-black flex items-center justify-center">
            <img src={resolveImagePath(image.imageDataUrl)} alt={`Image of ${displayName}`} className="max-w-full max-h-[90vh] object-contain" />
          </div>

          <div className="w-full md:w-1/3 p-6 flex flex-col overflow-y-auto">
            <h2 className="text-2xl font-bold mb-2 text-indigo-400">{displayName}</h2>
